...
class PoseCNN(nn.Module):
    ...
    def __init__(self, num_classes, num_units, train_slim=False, use_rgbd=False,
                 freeze_layers=False, train_translation=True, train_rotation=True,
                 hard_label_threshold=0.9, hard_label_sampling=0.0, hard_angle=5.0,
                 hough_label_threshold=100, hough_voting_threshold=10, hough_skip_pixels=10,
                 hough_inlier_threshold=0.9):
        '''
        Creates a new torch module with PoseCNN architecture.

        :param num_clases: number of classes that should be used
        :param num_units:
        :param train_slim: perform slim training
        :param use_rgbd: determines if RGBD images are used instead of RGB
        :param freeze_layers: determines if certain layers should be frozen
        :param train_translation: determines if the translation of objects should be trained
        :param train_rotation: determines if the rotation of objects should be trained
        :param hard_label_threshold:
        :param hard_label_sampling:
        :param hard_angle:
        :param hough_label_threshold:
        :param hough_voting_threshold:
        :param hough_skip_pixels:
        :param hough_inlier_threshold=0.9:
        '''
        ...
        # capture params for forward pass
        ...
        self.classifier = self.vgg16.classifier[:-1]
        if train_slim:
            dim_fc = 256
        ...
        self.hard_label = HardLabel(threshold=hard_label_threshold, sample_percentage=hard_label_sampling)
        self.dropout = nn.Dropout()

        if self.train_translation:
            ...
            # hough voting
            self.hough_voting = HoughVoting(is_train=0, skip_pixels=hough_skip_pixels, label_threshold=hough_label_threshold, \
                                            inlier_threshold=hough_inlier_threshold, voting_threshold=hough_voting_threshold, per_threshold=0.01)
            ...
            if self.train_rotation:
                ...
                self.pml = PMLoss(hard_angle=hard_angle)
    ...
    def set_hough_params(self, label_threshold, voting_threshold, skip_pixels, inlier_threshold):
        self.hough_voting.label_threshold = label_threshold
        self.hough_voting.voting_threshold = voting_threshold
        self.hough_voting.skip_pixels = skip_pixels
        self.hough_voting.inlier_threshold = inlier_threshold
    ...
    def forward(self, x, label_gt, meta_data, extents, gt_boxes, poses, points, symmetry):
        ...
        if self.train_translation:
            ...
            # hough voting
            self.hough_voting.is_train = 1 if self.training else 0
